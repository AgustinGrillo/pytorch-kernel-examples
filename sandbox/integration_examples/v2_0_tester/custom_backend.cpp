...
void quantize_tensor_per_tensor_affine_privateuse1(const at::Tensor &rtensor,
                                                   at::Tensor &qtensor,
                                                   double scale,
                                                   int64_t zero_point) {
  // do nothing
...
REGISTER_PRIVATEUSE1_DISPATCH(quantize_tensor_per_tensor_affine_stub,
                              &quantize_tensor_per_tensor_affine_privateuse1);

} // namespace at::native
struct CustomBackendMetadata : public c10::BackendMeta {
  // for testing this field will mutate when clone() is called by
  // shallow_copy_from.
  ...
  CustomBackendMetadata(int backend_version_format, int format_number)
      : backend_version_format_(backend_version_format),
        format_number_(format_number) {}
  c10::intrusive_ptr<c10::BackendMeta>
  clone(const c10::intrusive_ptr<c10::BackendMeta> &ptr) const override {
    cloned_ = true;
...
make_custom_storage_impl(c10::StorageImpl::use_byte_size_t,
...
at::Tensor custom_add_Tensor(const at::Tensor &self, const at::Tensor &other,
                             const at::Scalar &alpha) {
  add_counter += 1;
  // Since this custom device is just for testing, not bothering to implement
  // kernels.
...
struct DummyCustomAllocator final : at::Allocator {
  ...
  at::DataPtr allocate(size_t nbytes) override {
    ...
    return {data, data, &ReportAndDelete,
            at::Device(at::DeviceType::PrivateUse1, custom_device_index)};
  }
...
// basic dummy empty function, so we can directly construct tensors on the
// custom device This dummy test device will just use the CPU allocator, and
// ignores pinned memory.
...
// Unsafe using dummy device data_ptr to creat a cpu tensor, and shared
// data_ptr.
at::Tensor unsafe_create_cpu_tensor_from_dummy_tensor(const at::Tensor &src) {
  ...
  size_t size_bytes = at::detail::computeStorageNbytes(
      sizes_, strides_, src.element_size(), storage_offset_);

  at::DataPtr data_ptr = c10::InefficientStdFunctionContext::makeDataPtr(
      src.storage().mutable_data_ptr().get(), [](void *) {}, at::kCPU);

  c10::Storage storage{
      c10::Storage::use_byte_size_t{}, size_bytes, std::move(data_ptr),
...
// basic dummy copy_() function, so we can copy from the custom device to/from
// CPU
at::Tensor custom__copy_from(const at::Tensor &self, const at::Tensor &dst,
                             bool non_blocking) {
...
at::Tensor custom__copy_from_and_resize(const at::Tensor &self,
                                        const at::Tensor &dst) {
...
at::Tensor &custom_set_source_Storage(at::Tensor &result, c10::Storage src) {
  int64_t new_size =
      static_cast<int64_t>(src.nbytes() / result.dtype().itemsize());
  ...
  at::native::resize_impl_cpu_(result.unsafeGetTensorImpl(), new_size,
                               stride_opt,
...
at::Tensor custom__pin_memory(const at::Tensor &self,
                              std::optional<at::Device> device) {
  ...
  custom_pinned_data_ptr.push_back(
      dump_pinned_tensor.storage().data_ptr().get());

  return dump_pinned_tensor;
}

bool custom_is_pinned(const at::Tensor &self,
                      std::optional<at::Device> device) {
...
const at::Tensor &
custom_resize_(const at::Tensor &self, at::IntArrayRef size,
  ...
  const auto storage_size =
      at::detail::computeStorageNbytesContiguous(size, itemsize, offset);
...
// For open registration, we're registering all of our kernels to the
// PrivateUse1 dispatch key. Later in this file, we map a custom device to the
// PrivateUse1 device type, which allows user code that puts a tensor on your
// custom_device to eventually get plumbed into the kernels registered here.
//
...
TORCH_LIBRARY_IMPL(aten, PrivateUse1, m) {
  ...
  m.impl("set_.source_Storage_storage_offset",
         &custom_set_source_Storage_storage_offset);
...
// This basic implementation doesn't bother dealing with different device
// indices (e.g. custom_device:0 vs. custom_device:1). We could do that by
// letting the user pass in a device index in our exposed device function. Note
// that if you do that, you'll also need to register a device guard to core. See
// `c10/core/impl/DeviceGuardImplInterface.h:C10_REGISTER_GUARD_IMPL`.
...
struct FooHooksInterface : public at::PrivateUse1HooksInterface {
  ...
  const at::Generator &
  getDefaultGenerator(c10::DeviceIndex device_index) override {
...
TORCH_DECLARE_REGISTRY(PrivateUse1HooksRegistry, FooHooksInterface,
                       FooHooksArgs);
...
const at::Generator &default_generator(c10::DeviceIndex device_index) {
  return at::globalContext().defaultGenerator(
      at::Device(c10::DeviceType::PrivateUse1, device_index));
  ;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("custom_add_called", &custom_add_called,
        "check if our custom add function was called");
  m.def("register_generator_first", &register_generator_first,
        "register generator for custom device firstly");
  m.def("register_generator_second", &register_generator_second,
        "register generator for custom device secondly");
  m.def("set_custom_device_index", &set_custom_device_index,
        "set custom device index");
  m.def("custom_storage_registry", &custom_storage_registry,
        "set custom storageImpl creat method");
  m.def("custom_storageImpl_called", &custom_storageImpl_called,
        "check if our custom abs function was called");
  m.def("custom_set_backend_meta", &custom_set_backend_meta,
        "a fake set tensor BackendMeta function");
  m.def("check_backend_meta", &check_backend_meta,
        "check if BackendMeta serialization correctly");
  m.def("custom_serialization_registry", &custom_serialization_registry,
        "register custom serialization function");
  m.def("register_hook", &register_hook, "register_hook for privateuse1");
  m.def("default_generator", &default_generator,
        "default_generator for privateuse1");

  // Co-opting this file to more easily test torch.compile'ing of custom
  // autograd functions in C++
