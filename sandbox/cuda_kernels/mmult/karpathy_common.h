...
template <class ElementType> struct alignas(16) Packed128 {
  ...
  // so in the case where ElementType = bfloat16, we store 8 elements in one
  // Packed128
...
// CUBLAS_COMPUTE_32F or CUBLAS_COMPUTE_16F (for CUDA_R_16F only, potentially
// slower?!)
...
[[nodiscard]] cudaError_t memcpy_convert(TargetType *d_ptr, float *h_ptr,
  ...
  // instead of checking the status at cudaMemcpy, we return it from here. This
  // way, we still need to use our checking macro, and get better line info as
