...
It's advised to use OpenMP here because the CPU implementation is fairly slow
otherwise

Compile example:
nvcc -O3 --use_fast_math -Xcompiler -fopenmp matmul_forward.cu -o matmul_forward
-lcublas -lcublasLt

version 1 is naive port from CPU code to kernel: parallelizes over B,T, loops
over C OMP_NUM_THREADS=32 ./matmul_forward 1
...
void matmul_forward2(float *out, const float *inp, const float *weight,
  ...
  // now, because we use row-major storage, cuBLAS (which is column-major) sees
  // our matrices transposed. algorithmically / in e.g. PyTorch we want to do:
  // out = inp @ weight.T but because cuBLAS is column-major, we actually want
  // to get it to calculate out.T . Mathematically, this is: out.T = weight @
  // inp.T but again, our variables look transposed, so using the actual
  // weight/inp we have here in this function, this becomes out.T = weight.T @
  // inp so we need to get cuBLAS to calculate weight.T @ inp (the variables
  // here are the actual ones in this function)
...
int main(int argc, char **argv) {
  ...
  for (int j = 0; j < sizeof(sqrt_block_sizes) / sizeof(int); j++) {
    ...
    float elapsed_time =
        benchmark_kernel(repeat_times, matmul_forward, kernel_num, d_out, d_inp,
                         d_weight, d_bias, B, T, C, OC, sqrt_block_size);
